...
const CALL_TIMEOUT = 2000; // 2 seconds between attempts

// Add this early in the script to handle all social and phone links
document.addEventListener('DOMContentLoaded', function() {
    // Rate limit phone calls
    const phoneLink = document.querySelector('a[href^="tel:"]');
    if (phoneLink) {
        phoneLink.addEventListener('click', function(e) {
            const now = Date.now();
            if (now - lastCallAttempt < CALL_TIMEOUT) {
                e.preventDefault();
                console.warn('Please wait before trying to call again');
                return;
            }
            lastCallAttempt = now;
        });
    ...
    socialLinks.forEach(link => {
        ...
        link.setAttribute('href', sanitizeUrl(originalHref));
    });
});
...
window.addEventListener('load', function() {
    function updateOnlineStatus() {
        if (!navigator.onLine) {
            // Show a subtle notification that content might be limited
            ...
            offlineNotification.style.backgroundColor = 'rgba(255, 31, 90, 0.9)';
            offlineNotification.style.color = 'white';
            offlineNotification.style.padding = '10px 20px';
            offlineNotification.style.borderRadius = '30px';
            offlineNotification.style.zIndex = '1000';
            offlineNotification.style.boxShadow = '0 4px 15px rgba(0, 0, 0, 0.2)';
            offlineNotification.textContent = 'You are offline - some content may not be available';
            document.body.appendChild(offlineNotification);
            
            // Remove after 5 seconds
            setTimeout(() => {
                offlineNotification.style.transition = 'opacity 1s';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    heroImg.src = 'assets/images/avif/Anu.avif';
